...
Webhook endpoint UUIDs used in path parameters come from create responses or, preferably,
from list endpoints. The list responses include `endpoint.id`, which is the value to
use as `<ENDPOINT_UUID>` in subsequent calls (update, delete, address management,
